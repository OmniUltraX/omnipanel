//! 联网搜索 / 抓取配置与测试（与桌面端 `commands::web_search` 对齐）。
...
fn empty_proxy() -> omnipanel_store::HttpProxyConfig {
...
pub async fn web_search_test_backend(backend: String) -> Result<WebSearchTestResultDto, OmniError> {
...
pub async fn web_search_test_fetch(url: String) -> Result<WebFetchTestResultDto, OmniError> {
    let proxy = empty_proxy();
    let target = url.trim();
    if target.is_empty() {
