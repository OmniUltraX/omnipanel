...
use crate::db_ops;
use crate::event::{
    BgTaskDbEvent, DbSyncEventSink, SchemaColumnDiffPayload, SchemaCompareEvent,
...
async fn compare_schema_for_table(
    ...
    match db_ops::db_introspect_table_op(target, Some(target_schema), target_table_name).await {
        Ok(target_table) => {
...
async fn ensure_table_from_source(
    ...
    let ddl = db_ops::db_table_ddl_op(
        source.clone(),
...
async fn copy_table_data_with_modes(
    ...
    let source_total = db_ops::db_count_table(source_conn.clone(), None, table.to_string(), None)
        .await
        .unwrap_or(0.0)
        .max(0.0) as u32;
    let target_total = db_ops::db_count_table(target_conn.clone(), None, table.to_string(), None)
        .await
        .unwrap_or(0.0)
        .max(0.0) as u32;
    ...
    if modes.insert || modes.merge {
        ...
        while offset < i64::from(source_total.max(1)) || (source_total == 0 && offset == 0) {
            ...
            let page = db_ops::db_preview_table(
                source_conn.clone(),
                table.to_string(),
                PAGE_SIZE as u32,
                offset as u32,
...
pub async fn generate_data_sync_sql_script(
    ...
    let sql = format!("{}\n", lines.join("\n").trim_end());
    let file_path = crate::paths::save_sync_sql_file(&sql)?;
...
pub async fn run_db_data_sync_sql_file_execute(
    ...
    let driver = db_ops::open_db_driver(&target_conn).await?;
...
async fn resolve_exec_table_columns(
    ...
    let schema = db_ops::db_introspect_table_op(
        source.clone(),
        Some(source_db.to_string()),
        spec.name.clone(),
    )
...
async fn collect_schema_diff_sql(
    ...
    let target_table = db_ops::db_introspect_table_op(
        target.clone(),
