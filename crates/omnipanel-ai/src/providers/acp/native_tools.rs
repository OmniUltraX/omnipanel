...
//! - Read/Write/Edit/Find/Grep: 映射为 `omni_terminal_exec`（用 cat/echo/sed/find/grep 在当前会话执行）
...
/// 将原生 Read 工具映射为 `omni_terminal_exec { command: "cat path" }`。
...
/// 将原生 Write 工具映射为 `omni_terminal_exec { command: "cat > path <<'EOF'...EOF" }`。
...
/// 将原生 Edit 工具映射为 `omni_terminal_exec`（用 perl -i 实现跨平台替换）。
...
/// 将原生 Find 工具映射为 `omni_terminal_exec { command: "find path -name query" }`。
...
/// 将原生 Grep 工具映射为 `omni_terminal_exec { command: "grep -rn pattern path" }`。
