...
use omnipanel_plugin::{load_installed, PluginRegistry, PluginSource};
...
fn l1_sample_installs_and_contributes() {
    ...
    // 1. 打包样板（模拟第三方开发者产出）
    let entries = BTreeMap::from([("plugin.json".to_string(), SAMPLE_MANIFEST.as_bytes().to_vec())]);
    ...
    let sample = list.iter().find(|i| i.id == "omni.engine.l1-starter").unwrap();
    ...
    // 5. 禁用后贡献点消失，连接数据无关
    registry.set_enabled("omni.engine.l1-starter", false).unwrap();
    assert!(!registry
        .contributions()
        .ai_tools
        .iter()
...
fn l1_sample_builtin_id_collision_keeps_builtin() {
    ...
    let mut registry = PluginRegistry::new();
    registry.register(omnipanel_plugin::engine_qdrant()).unwrap();
    registry
        .register_installed(
