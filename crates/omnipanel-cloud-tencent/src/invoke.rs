use omnipanel_cloud_aliyun::{
    AliyunCredentials, CloudAction, CloudLogQuery, CloudMetricQuery, CloudProviderDriver,
    CloudResourceFilter,
};
...
struct InvokeCredentials {
    #[serde(default, alias = "secretId", alias = "SecretId")]
    access_key_id: String,
    #[serde(default, alias = "secretKey", alias = "SecretKey")]
...
fn parse_creds(args: &Value) -> Result<AliyunCredentials, OmniError> {
    ...
    if secret_id.is_empty() || secret_key.is_empty() {
        return Err(OmniError::invalid_input("缺少 SecretId / SecretKey"));
    }
...
/// InvokeGateway 入口：解析 JSON 参数并调用腾讯云 Driver。凭据只存在此次调用栈。
...
mod tests {
    ...
    fn declared_methods_whitelist() {
        ...
        assert!(is_declared_method("queryLogs"));
        assert!(!is_declared_method("cloudListCvm"));
        assert!(!is_declared_method("deleteInstance"));
    }

    #[test]
    fn parse_secret_id_key() {
        ...
        let creds = parse_creds(&args).expect("creds");
        assert_eq!(creds.access_key_id, "AKIDxxx");
