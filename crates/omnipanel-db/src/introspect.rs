//! Schema introspection / DDL / metadata (ported from desktop database commands).
...
pub async fn db_list_databases(connection: DbConnectionConfig) -> Result<Vec<String>, String> {
    match connection.db_type.to_lowercase().as_str() {
        ...
        "redis" => {
            ...
            crate::redis_list_databases(&to_params(&connection), &preset)
                .await
                .map_err(err_msg)
        }
        "postgresql" | "postgres" => {
...
pub async fn db_list_databases_with_stats(
    ...
    match connection.db_type.to_lowercase().as_str() {
        ...
        "qdrant" => {
            let infos = crate::qdrant_list_collection_infos(&to_params(&connection))
                .await
                .map_err(err_msg)?;
...
async fn introspect_qdrant_collection(
    ...
    let driver = crate::connect(&to_params(connection))
        .await
        .map_err(err_msg)?;
    let result = driver
...
/// 列出指定库下的表名（与桌面端 `db_list_tables` 命令同形）。
...
pub async fn list_database_objects_shallow(
    connection: &DbConnectionConfig,
    db_name: &str,
) -> Result<crate::schema_refresh::SchemaCacheDatabasePayload, String> {
    use crate::schema_refresh::SchemaCacheDatabasePayload;
