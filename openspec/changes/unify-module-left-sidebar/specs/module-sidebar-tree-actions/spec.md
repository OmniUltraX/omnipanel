...
每棵树所在 L1 段头的 actions 区 SHALL 按固定顺序提供：新建（`+`，无新建能力的树不渲染）→ 刷新 → 一键展开 → 一键折叠；按钮 SHALL 为 22px 幽灵图标按钮；无对应能力的树 SHALL 将按钮置灰而非缺失（保证各模块段头按钮位对齐）；折叠/展开 SHALL 只作用于本段树节点，不折叠 L1 分段本身。

#### Scenario: 一键折叠与禁用态

- **WHEN** 用户点击数据库段头的折叠按钮
- **THEN** 本段树所有节点收起（`expandedSize===0`），按钮进入禁用态；点击展开后恢复，禁用态切换到展开按钮
