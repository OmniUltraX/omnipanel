...
- **插件 · 翻译**：打包 CSP 增加 `script-src 'unsafe-eval'`，动态 `ui/main.js` 可求值，消除启动时 `ui.invalid_entry`
- **发版 · OSS**：大文件改分片上传并延长超时、失败重试；提供可手动补传的 `publish-oss` 工作流

## [0.8.40] - 2026-09-24

### 新增

- **侧栏 · 备注**：连接根与叶子节点可本地备注（Docker / SSH / 服务器 / 云 / 文件 / 协议 / 知识库 / 终端 / 数据库），不写远端
- **数据库 · SQL**：按语句保存执行日志；编辑器框选当前语句或最小子查询，行号三角可单独运行
- **Docker · 预览 Tab**：右侧 Dock 双击预览标签可常驻打开

### 修复

- **同步 · 自动传钥**：打包版经 IPC 代理 omniserver，避免 WebView CORS 导致「拉取待中继请求失败 (Failed to fetch)」
- **Docker · 容器日志**：开启跟踪时先挂监听再开流，避免 `--tail` 历史被清空成空白；流式追加不再误全量 reset
- **Docker · Compose（SSH）**：日志截断按 UTF-8 字符边界，不再因中文切半 panic
- **安装包图标**：重新生成多尺寸 PNG / ICO / ICNS，修复正式包与 DEV 图标资源损坏

## [0.8.33] - 2026-09-23

### 新增

- **数据库 · SQL**：编辑器右键菜单（执行、新结果页、导出、注释、大小写、简单格式化、发送到 AI）；新建查询直接用默认名打开，F2 重命名
- **数据库 · SQL**：简单格式化按关键字换行并给子查询缩进；完整格式化保留语句前后的空行
- **数据库 · 连接树**：展开的连接、库和分组（如表）在滚动表列表时吸顶
- **库同步**：行差异改走 rusqlite 流式落盘，大表对比不再整表进内存

### 修复

- **数据库 · SQL**：语句前面的行注释或块注释不再把 SELECT 当成非查询，结果集能正常返回
- **数据库 · 日志**：二进制日志与慢日志可打开；按时间从新到旧加载，选中行不会被后续刷新顶走；慢日志走连接池并虚拟分页
- **窗口**：左右或上下分屏后，窗口控制按钮挂回正确的分组
...
- **同步 · 插件**：拉取模块快照后按本机资源自动安装官方 / 已用 DBX 插件；第三方市场插件需确认；缺件不阻断同步
- **云厂商**：侧栏对齐标准分组工具条（刷新 / 展开收起）；插件未就绪时状态点不再误报离线

### 性能

- **模块切换**：悬停只预拉 chunk 与文案、不再挂壳；隐藏叠层跳过布局；闲时不再全量扫模块与文案重渲

### 修复

- **发版 CI**：Web Docker 镜像构建补齐 `packages/` 与 `plugins/`（tsc 能解析 plugin-sdk）；IPC 双清单补齐 handler-only 白名单
- **快捷启动**：看板 slash 列表按标签排序的断言与实现对齐
...
- **云厂商插件**：八家云厂商（阿里云 / 腾讯 / 华为 / AWS / Azure / DO / GCP / 搬瓦工）全部改为 L2 `logic.js` + 市场 download-only，源码挂独立仓 submodule；升级后需从插件中心安装所需厂商，已有连接的 `pluginId` 不变。Web 版暂无云插件运行时
- **模块 · Nacos**：改为 download-only 模块插件（独立仓 + submodule），不再随客户端捆绑
- **知识库 · 思源**：只走知识类插件同步，移除宿主原生思源路径

### 新增

- **云 · 阿里云**：安全组支持删除（插件 `0.1.1`）；详情 / 列表入口 + 在场验证
- **知识库**：思源 S3 导入、图片与标签同步、数据源 Tabs；文档可挂子文档
- **插件工作室**：以工程为中心；目录导入为链接工程；本地目录安装与热重载；接 AI Dock；本地开发版标注
- **插件市场**：Rubick 第三方源与全量分类；外部插件适配安装与兼容判定；翻译类插件转市场下载
- **数据库**：表数据筛选逐行 AND/OR、拖拽排序与停用保留；多列排序；连接 / 配置档管理增强；CSV 导出与网格重构
- **协议 / 面板**：新增 `omnipanel-protocol` / `omnipanel-panel` crate；面板站点配置与证书绑定增强
- **壳层**：后台静默数据预热；系统应用枚举与启动；看板 catalog 同步与 slash 命令
- **侧栏**：小程序助手管理与同步密钥入口增强

### 修复

- **插件注册表**：兼容 v1 形态条目解析，避免市场丢包
- **云插件打包**：download-only 清单改为运行时读盘，独立仓 pack CI 不再依赖全部 submodule
- **壳层 / Dock**：侧栏切换高亮回闪；分屏拖拽与空值过滤；后台预热真实延迟，解启动 stampede
- **插件 UI**：选中悬浮按钮层级与压过单元格编辑器；联网修正与内网地址防护

### 性能

- **壳层**：idle 扫描只预拉 chunk，挂壳走意图驱动
...
- **快捷启动 · 询问 AI**：面板内 HTTP 流式问答（不跳主窗 AI 抽屉）；展示当前助手场景模型；Markdown 渲染与流式跟随滚动
- **快捷启动 · 询问记录**：本地持久化问答历史；非收藏最多 5 条；支持收藏 / 删除；点击只读回看，不重新请求
- **插件 · 工作台与投稿**：工作室收成可跑开发闭环（校验 / 打包 / 安装）；官方验签与投稿链路；多源安装与环境一键装
- **模块窗 · 空闲卸载**：独立模块窗隐藏后 10 分钟未再打开则 `destroy` 释放 WebView；再次打开冷启动

### 修复

- **运行时 · 保活**：路由切换时立即同步 keep-alive，避免 Overlay 切回不渲染
- **团队同步 · 布局**：推送 / 拉取前等待侧栏布局 store hydration；远端空文件夹不覆盖本机，避免空布局误推云端
...
- **插件市场**：多源注册表与版本安装 IPC；市场地基与插件工作台；第三方插件交付，面板 Host 只认 driver
- **模块宿主 · Nacos**：服务发现展示在线/离线，实例支持上线/下线；左右分栏可拖动（默认 30%/70%）；Ctrl/Cmd+F 聚焦「搜索配置」
- **宝塔面板**：已装应用展示版本与端口；已装列表与网页对齐；MySQL / Redis 一键管理参数解析
- **工作台 UI**：统一幽灵操作与扁平按钮（页头 / 空态 / 对话框）
- **运行时**：模块保活与秒切回；Overlay keep-alive；看板隐藏即停轮询
- **Docker**：SSH 导入支持用户自定义连接模式
- **工程**：i18n / 数据库模块结构优化；HTML 纯文本剥离工具

### 修复

- **Nacos · 配置发布**：发布后不再清空列表选中与编辑器正文（刷新保留选中；仅命名空间变更触发侧栏 reload）
- **Nacos · 配置列表**：`listConfigs` 搜索只用 keyword，避免误用单条 dataId/group 滤空
- **工程**：消除 `cargo check` 未使用 import 警告（`marketplace.rs`）
...
- **模块插件 · Nacos**：`kind=module` 可安装插件启用后出现侧栏；连接树 + 多 Tab 工作台；在当前连接上切换命名空间（对齐云厂商切地区），配置 / 服务 / 集群按空间刷新
- **云 · 腾讯云**：独立 crate + 官方插件，能力工作台与阿里云同一套账户 → 能力 → 实例
- **云 · 阿里云资源**：在 ECS / 轻量 / OSS / 域名 / 证书之外补齐安全组、EIP、负载均衡、RDS、云数据库 Redis、磁盘与 DNS
