# omni-plugin-cloud-azure

OmniPanel 可选插件：`omni.cloud.azure`（Azure）。
...
cargo run -p omnipanel-plugin-pkg --bin pack -- \
  plugins/cloud-azure \
  omni-cloud-azure-0.1.0.omni-plugin
```
...
宿主仓库以 git submodule 挂载于 `plugins/cloud-azure`。
...
制品文件名固定：`omni-cloud-azure-<version>.omni-plugin`。
