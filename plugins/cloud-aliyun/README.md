# omni-plugin-cloud-aliyun

OmniPanel 可选插件：`omni.cloud.aliyun`（阿里云）。
...
cargo run -p omnipanel-plugin-pkg --bin pack -- \
  ...
  omni-cloud-aliyun-0.1.0.omni-plugin
```
...
宿主仓库以 git submodule 挂载于 `plugins/cloud-aliyun`。
...
制品文件名固定：`omni-cloud-aliyun-<version>.omni-plugin`。
