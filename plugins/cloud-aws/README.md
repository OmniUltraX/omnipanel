# omni-plugin-cloud-aws

OmniPanel 可选插件：`omni.cloud.aws`（AWS）。
...
cargo run -p omnipanel-plugin-pkg --bin pack -- \
  plugins/cloud-aws \
  omni-cloud-aws-0.1.0.omni-plugin
```
...
宿主仓库以 git submodule 挂载于 `plugins/cloud-aws`。
...
制品文件名固定：`omni-cloud-aws-<version>.omni-plugin`。
