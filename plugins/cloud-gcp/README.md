# omni-plugin-cloud-gcp

OmniPanel 可选插件：`omni.cloud.gcp`（GCP）。
...
cargo run -p omnipanel-plugin-pkg --bin pack -- \
  plugins/cloud-gcp \
  omni-cloud-gcp-0.1.0.omni-plugin
```
...
宿主仓库以 git submodule 挂载于 `plugins/cloud-gcp`。
...
制品文件名固定：`omni-cloud-gcp-<version>.omni-plugin`。
