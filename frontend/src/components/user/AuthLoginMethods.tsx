...
function EmailLoginPanel() {
  ...
  const onSendCode = useCallback(async () => {
    ...
    try {
      ...
      // 重发冷却取短窗口，避免误用验证码 TTL（常为数分钟）
      setCooldown(Math.min(60, Math.max(1, sent.expireInSec || 60)));
      if (sent.code) {
        setCode(sent.code);
        setHint(
          sent.hint
...
export interface AuthLoginMethodsProps {
  /** 是否显示卡片标题（个人中心嵌入时建议开启） */
  showTitle?: boolean;
  className?: string;
}

/**
 * 多方式登录主体：微信扫码 / GitHub OAuth / 邮箱验证码。
 * 供启动门禁登录页与个人中心退出后复用。
 */
export function AuthLoginMethods({ showTitle = false, className }: AuthLoginMethodsProps) {
  const { t } = useI18n();
  const githubIcon = useGithubIcon();
  const [method, setMethod] = useState<LoginMethod>("wechat");

  const rootClass = ["auth-login-methods", className].filter(Boolean).join(" ");

  return (
    <div className={rootClass}>
      {showTitle ? (
        <>
          <h2 className="auth-login-methods__title">{t("app.login.title")}</h2>
          <p className="auth-login-methods__desc">{t("app.login.desc")}</p>
        </>
      ) : null}
