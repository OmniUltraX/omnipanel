...
export interface SubWindowProps {
  ...
  onClose: () => void;
  children: ReactNode;
  /** 相对主窗坣坯视区域的宽度比例，默�?0.9 */
  widthRatio?: number;
  /** 相对主窗坣坯视区域的高度比例，默�?0.9 */
  heightRatio?: number;
  className?: string;
  /** 标题与关闭按钮之间的附加控件（如模型选择�?*/
  headerExtra?: ReactNode;
  /** 是坦坯用拖动〝缩放与最大化/最尝化，默�?true */
  windowChrome?: boolean;
  /** 是坦隝藝靮罩层，默认 false */
  noOverlay?: boolean;
  /** 自定义最尝化行为，丝传则使用内置最尝化（坳侧浮动条�?*/
  onMinimize?: () => void;
  /** 当杝供时，最大化按钮改为切杢到全屝工作区模弝（关闭弹窗并激�?tab�?*/
...
export function SubWindow({
  ...
  const handleHeaderDoubleClick = useCallback(
    (event: React.MouseEvent<HTMLDivElement>) => {
      ...
      if (onMaximizeToWorkspace) {
        onMaximizeToWorkspace();
        return;
      }
      handleToggleMaximize();
    },
    [handleToggleMaximize, onMaximizeToWorkspace, windowChrome],
  );
