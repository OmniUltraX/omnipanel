...
export type PersistedModuleTabOptions<T extends string> = {
  /** 旧 Tab id → 现行合法 Tab（回读时迁移并写回） */
...
export function usePersistedModuleTab<T extends string>(
  ...
  options?: PersistedModuleTabOptions<T>,
): [T, Dispatch<SetStateAction<T>>] {
  const stored = useModuleTabStore((s) => s.byModule[moduleKey]);
  const aliases = options?.aliases;

  const aliased =
    stored && validTabs && !validTabs.includes(stored as T) && aliases?.[stored]
      ? aliases[stored]
      : null;
  ...
  useEffect(() => {
    if (aliased && stored !== aliased) {
      useModuleTabStore.getState().setTab(moduleKey, aliased);
    }
  }, [aliased, moduleKey, stored]);
