...
function ansiQuoteId(name: string): string {
  return `"${name.replace(/"/g, '""')}"`;
}

function ansiTableRef(dbName: string, tableName: string): string {
  const table = tableName.trim();
  const db = dbName.trim();
  if (db) return `${ansiQuoteId(db)}.${ansiQuoteId(table)}`;
  return ansiQuoteId(table);
}

function ansiColumnType(field: TableDesignerFieldRow): string {
  let type = field.type.trim();
  if (field.length.trim()) {
    type += `(${field.length.trim()})`;
  }
...
function ansiColumnDef(field: TableDesignerFieldRow): string {
  let def = `${ansiQuoteId(field.name.trim())} ${ansiColumnType(field)}`;
  if (!field.nullable) def += " NOT NULL";
  if (field.isPk) def += " PRIMARY KEY";
  if (field.defaultValue.trim()) def += ` DEFAULT ${field.defaultValue.trim()}`;
  return def;
...
function oracleColumnType(field: TableDesignerFieldRow): string {
  let type = field.type.trim();
  if (field.length.trim() && /varchar2|nvarchar2|char|number|raw/i.test(type)) {
    type += `(${field.length.trim()})`;
  }
  return type;
}

function oracleColumnDef(field: TableDesignerFieldRow): string {
  let def = `${ansiQuoteId(field.name.trim())} ${oracleColumnType(field)}`;
  if (!field.nullable) def += " NOT NULL";
  if (field.isPk) def += " PRIMARY KEY";
  if (field.isAutoIncrement) def += " GENERATED BY DEFAULT AS IDENTITY";
  if (field.defaultValue.trim()) def += ` DEFAULT ${field.defaultValue.trim()}`;
  return def;
}

export function buildCreateSqlOracle(model: TableDesignerModel, dbName: string): string[] {
  const tableName = model.tableName.trim();
  if (!tableName) return [];
  const tableRef = ansiTableRef(dbName, tableName);
  const lines = model.fields.map((field) => oracleColumnDef(field));
  const stmts: string[] = [`CREATE TABLE ${tableRef} (\n  ${lines.join(",\n  ")}\n)`];
  for (const index of model.indexes) {
    if (index.primary || index.columns.length === 0) continue;
    const cols = index.columns.map((c) => ansiQuoteId(c.trim())).join(", ");
    const name = ansiQuoteId(index.name.trim() || `idx_${index.columns.join("_")}`);
    const unique = index.unique ? "UNIQUE " : "";
    stmts.push(`CREATE ${unique}INDEX ${name} ON ${tableRef} (${cols})`);
  }
  if (model.comment.trim()) {
    stmts.push(`COMMENT ON TABLE ${tableRef} IS '${escapeSqlString(model.comment.trim())}'`);
  }
  return stmts;
}

export function buildApplySqlOracle(
  baseline: TableDesignerModel,
  model: TableDesignerModel,
  dbName: string,
): string[] {
  if (isNewTableBaseline(baseline)) {
    return buildCreateSqlOracle(model, dbName);
  }

  const stmts: string[] = [];
  const tableName = model.tableName.trim() || baseline.tableName.trim();
  const tableRef = ansiTableRef(dbName, tableName);
  const baseTableName = baseline.tableName.trim();
  if (baseTableName && tableName && baseTableName !== tableName) {
    stmts.push(
      `ALTER TABLE ${ansiTableRef(dbName, baseTableName)} RENAME TO ${ansiQuoteId(tableName)}`,
    );
  }
  ...
  for (const field of baseFields.values()) {
    if (!curFields.has(field.id)) {
      stmts.push(`ALTER TABLE ${tableRef} DROP COLUMN ${ansiQuoteId(field.name.trim())}`);
    }
  }

  for (const field of curFields.values()) {
    if (!baseFields.has(field.id)) {
      stmts.push(`ALTER TABLE ${tableRef} ADD (${oracleColumnDef(field)})`);
    }
  }
  ...
  for (const field of curFields.values()) {
    ...
    if (base.name.trim() !== field.name.trim()) {
      stmts.push(
        `ALTER TABLE ${tableRef} RENAME COLUMN ${ansiQuoteId(base.name.trim())} TO ${ansiQuoteId(field.name.trim())}`,
      );
    }
    if (
      base.type.trim() !== field.type.trim() ||
      ...
      stmts.push(`ALTER TABLE ${tableRef} MODIFY (${oracleColumnDef(field)})`);
    }
    if (base.comment.trim() !== field.comment.trim()) {
      stmts.push(
        `COMMENT ON COLUMN ${tableRef}.${ansiQuoteId(field.name.trim())} IS '${escapeSqlString(field.comment.trim())}'`,
      );
    }
  }
  ...
  for (const index of baseIdx.values()) {
    ...
    if (!cur || indexSignature(index) !== indexSignature(cur)) {
      const name = index.name.trim() || `idx_${index.columns.join("_")}`;
      stmts.push(`DROP INDEX ${ansiQuoteId(name)}`);
    }
  }
...
export function buildCreateSqlAnsi(model: TableDesignerModel, dbName: string): string[] {
  const tableName = model.tableName.trim();
  if (!tableName) return [];
  const tableRef = ansiQuoteId(tableName);
  const lines = model.fields.map((field) => ansiColumnDef(field));
  const stmts: string[] = [`CREATE TABLE ${tableRef} (\n  ${lines.join(",\n  ")}\n)`];
  for (const index of model.indexes) {
    if (index.primary || index.columns.length === 0) continue;
    const cols = index.columns.map((c) => ansiQuoteId(c.trim())).join(", ");
  ...
  if (dbName.trim()) {
    stmts.unshift(`-- ${dbName.trim()}.${tableName}`);
  }
  return stmts;
}

export function buildApplySqlAnsi(
  baseline: TableDesignerModel,
  model: TableDesignerModel,
  dbName: string,
): string[] {
  if (isNewTableBaseline(baseline)) {
    return buildCreateSqlAnsi(model, dbName);
  }
  ...
  const baseTableName = baseline.tableName.trim();
  if (baseTableName && tableName && baseTableName !== tableName) {
    stmts.push(`ALTER TABLE ${ansiQuoteId(baseTableName)} RENAME TO ${ansiQuoteId(tableName)}`);
  }
  ...
  for (const field of curFields.values()) {
    if (!baseFields.has(field.id)) {
      stmts.push(`ALTER TABLE ${tableRef} ADD ${ansiColumnDef(field)}`);
    }
  }
