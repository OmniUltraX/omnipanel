...
import { type TableDataGridCellMenuState } from "./TableDataGridChrome";
import {
...
import { useTableDataGridPointerInteraction } from "./useTableDataGridPointerInteraction";
import { useTableDataGridColumnDefs } from "./useTableDataGridColumnDefs";

export type TableDataGridViewModel = NonNullable<ReturnType<typeof useTableDataGridModel>>;

/**
 * TableDataGrid 逻辑编排（方案 A 组装中枢）。
 * 指针拖选已抽到 useTableDataGridPointerInteraction；其余切面按刀序继续外提。
 */
export function useTableDataGridModel(props: TableDataGridProps) {
  const {
    columns,
    rows,
    totalRows,
  ...
  const { columnDefs } = useTableDataGridColumnDefs({
    displayColumns,
    transposed,
    columnMetaMap,
    columnRelations,
  ...
  const { beginRowResize } = useTableDataGridPointerInteraction({
    clearGridSelection,
    wrapRef,
    canvasBodyRef,
    cellRangeRef,
