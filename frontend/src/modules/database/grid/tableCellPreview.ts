...
import {
  ...
  resolvePreviewMediaFromText,
} from "../../../lib/contentPreview";
import {
  formatOmniBlobDisplayText,
  parseOmniBlobValue,
  resolveOmniBlobPreviewContent,
} from "./omniBlobValue";
...
export function computeCellOverlayMaxWidth(viewportWidth = window.innerWidth): number {
  return Math.min(
    ...
    viewportWidth * CELL_PREVIEW_DRAWER_VIEWPORT_WIDTH_RATIO - CELL_OVERLAY_VIEWPORT_MARGIN * 2,
  );
}
...
export function buildCellPreviewState(cell: {
  column: string;
  rowIndex: number;
  row: Record<string, unknown>;
  value: unknown;
...
export type CellPreviewContent = ContentPreviewPayload;
...
export function resolveCellPreviewCodeLanguage(
  ...
  if (content.kind === "image" || content.kind === "audio") return undefined;
...
/** 解析单元格预览内容：BLOB / 媒体 / JSON / 纯文本。 */
export function resolveCellPreviewContent(
  value: unknown,
  columnType?: string,
  options?: { sniffContent?: boolean },
): CellPreviewContent {
  const blob = parseOmniBlobValue(value);
  if (blob) {
    return resolveOmniBlobPreviewContent(blob);
  }
  ...
  if (typeof value === "string") {
    const media = resolvePreviewMediaFromText(value);
    if (media) {
      return media;
    }

    const trimmed = value.trim();
    const sniff = options?.sniffContent === true;
    const tryJson =
      isJsonColumnType(columnType) ||
      (sniff && (trimmed.startsWith("{") || trimmed.startsWith("["))) ||
...
const CELL_OVERLAY_FONT =
  '12px "Geist Variable", "PingFang SC", "Hiragino Sans GB", "Microsoft YaHei UI", "Microsoft YaHei", "Segoe UI", system-ui, sans-serif';
...
export function estimateCellOverlayDisplayWidth(
  ...
  const contentWidth = measureCellOverlayTextWidth(text) + CELL_OVERLAY_HORIZONTAL_CHROME;
  if (contentWidth <= 0) {
    return cellWidth;
  }
