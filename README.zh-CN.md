...
| **服务器管理** | 主机监控；**宝塔 / 1Panel**（网站、应用、证书、计划任务）；**云厂商**（阿里云 / 腾讯云；在账号上切换地区） |
...
| **插件工作台** | 不出工作台完成第三方插件的创建 / 编辑 / 校验 / 打包 / 安装；AI 生成骨架 |
...
| 类别 | 说明 |
|------|------|
| **插件平台** | `.omni-plugin` 签名安装；插件中心（官方目录 + 第三方多源 / 版本安装）；L1 / L2 / L3；Host SDK 与开发者文档 |
