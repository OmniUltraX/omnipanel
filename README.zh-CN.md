...
| 类别 | 说明 |
|------|------|
| **团队 · OSS 同步** | 邮箱邀请成员、团队数据预览、同步排除项、自定义面板分享给团队成员 |
| **SSH · 进程监控** | 主机概览与进程轮询；进程详情命令行解析与缓存 |
| **Redis 控制台** | 应用内 Redis 控制台，运维面板体验优化 |
| **自定义监控面板** | 网格拖拽布局；主机 / Docker / MySQL / Redis 等监控小组件 |
| **数据库** | 表网格列标题统一；设置页字体下拉不再导致自定义标题栏消失 |
