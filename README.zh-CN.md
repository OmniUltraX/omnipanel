...
| 类别 | 说明 |
|------|------|
| **团队同步** | 侧栏切换同步团队；快照端到端加密；密码走 vault，不进 modules |
| **表数据预览** | WHERE / ORDER BY 按表记忆历史，↑/↓ 快速回填 |
| **终端** | 输出路径可点击打开；思考卡 / IME / 关 Tab 等稳定性修复 |
| **SQL 编辑器** | Ctrl/Cmd+点击表名打开数据面板；Ctrl/Cmd+滚轮缩放字号 |
| **自定义面板** | 关闭 Tab 不再删除，可从「+」菜单再打开 |
